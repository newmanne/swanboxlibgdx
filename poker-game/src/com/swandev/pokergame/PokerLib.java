...
public class PokerLib {
	...
	/* 
	 * Note: Many messages from the server to the client come with
	 * a triplet of integers representing the player's chip $ (how much
	 * they have to bet with), the players bet $ (how much they've already
	 * bet) and the current call $.
	 * 
	 ...
	 * Sender: PokerServer
	 * Receiver: HandScreen
	 * Purpose: Initialize a new hand of cards
	 * Additional data:
	 * 	- card IDs 	(two Integers)
	 ...
	 * Sender: PokerServer
	 * Receiver: HandScreen
	 * Purpose: Enable the action buttons on the client side, wait for response on the server side
	 * Additional data:
	 * 	- call $	(Integer)
	 * 
	 * Note: This could, in theory, send the entire $_state but it's not possible for that to have
	 * changed since the last message in the actual flow so it would only be for code simplicity.
	 */
	
	public static final String BET_REQUEST = "bet_request";
	/*
	 * Sender: HandScreen
	 * Receiver: PokerServer
	 * Purpose: Indicate to the server that a player would like to make a certain bet.
	 * Additional data:
	 * 	- bet $		(Integer)
	 ...
	 * Sender: HandScreen
	 * Receiver: PokerServer
	 * Purpose: Indicate to the server that a player wants to fold.
	 * Additional data:
	 * 	none
	 */
	 ...
	 * Sender: PokerServer
	 * Receiver: HandScreen
	 * Purpose: Indicate that the action requested (bet or fold) was valid and
	 * refresh the client's player state. After this, the client will be disabled
	 * until the next YOUR_TURN and the server will continue round-robin style.
	 * Additional data:
	 * 	- $_state	(three Integers)
	 */
	 ...
	 * Sender: PokerServer
	 * Receiver: HandScreen
	 * Purpose: Indicate that the action requested was invalid and the client should
	 * input another action. Client will be re-enabled and the server will wait for another
	 * request (bet/fold).
	 * Additional data:
	 * 	none
	 * 
	 * Note: Technically this should never happen as the invalid buttons should be disabled on the client
	 * side if they would generate an invalid action. Just here as a failsafe.
	 */
	 ...
	 * Sender: PokerServer
	 * Receiver: HandScreen
	 * Purpose: Indicate that the hand is over and refresh the client's PlayerState. Also indicate to the
	 * players whether they won or lost; presumably some animation/pop-up will occur on the client side in response.
	 * The Client will wait for the next DEAL_HAND message, and the server begins a new hand.
	 * Additional data:
	 * 	- $_state	(three Integers)
	 * 	- you_won 	(boolean)
	 */
